...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            dist[particle] += self.getObservationProb(observation, pacmanPosition, particle, self.getJailPosition())
            if dist[particle]!=0:
                flag = False

        if flag:
            self.initializeUniformly(self.particles)

        else:
            temp = list()
            for i in range(self.numParticles):
                temp.append(dist.sample())
            self.particles = temp
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                temp *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
            dist[particle] += temp
            if dist[particle] != 0:
                flag = False
